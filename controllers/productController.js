...
const getProducts = async  (req, res) => {
    ...
    try {
        ...
        res.render(createPath('products'), { products: productsWithRatings, title, username });
    } catch (error) {
        console.error(error);
...
const getProductById = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).render(createPath('error'), { title: 'Error', message: 'Product not found' });
    }
}
...
const editProductGet = (req, res) => {
    ...
    Product.findById(req.params.id)
        .then((product) => res.render(createPath('edit-product'), { product, title }))
        .catch((error) => {
            console.log(error);
            res.render(createPath('error'), { title: 'Error' });
        });
...
const editProductPut = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.render(createPath('error'), { title: 'Error', message: 'Failed to update product' });
    }
}
...
const addProductPost = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).render(createPath('error'), { title: 'Error', message: 'Failed to add product' });
    }
};
...
const searchProducts = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.render(createPath('error'), { title: 'Error', message: 'Failed to search products' });
    }
};
...
const getCart = async (req, res) => {
    ...
    try {
        ...
        if (!cart) {
            return res.render(createPath('cart'), { title, username, products: [], totalCost: 0 });
        }
        ...
        res.render(createPath('cart'), { title, username, products: productsWithDetails, totalCost });
    } catch (error) {
        console.error(error);
        res.status(500).render(createPath('error'), { title: 'Error', message: 'Failed to load cart' });
    }
};
const addToCart = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).render(createPath('error'), { title: 'Error', message: 'Failed to add product to cart' });
    }
};
